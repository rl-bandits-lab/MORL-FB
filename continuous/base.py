...
class MOMemory:
    ...
    def sample(self, batch_size, eval=False, pref=np.array([0.5, 0.5])):
        prob = []
        if eval:
            # calculate the cosine similarity between the preference and the preference of the samples
            for i in range(self._n):
                prob.append(np.dot(self.preferences[i], pref) /
                           (np.linalg.norm(self.preferences[i]) * np.linalg.norm(pref)))

            prob = np.exp(prob) / (np.sum(np.exp(prob)) + 1e-6)
            indices = np.random.choice(self._n, batch_size, p=prob)
